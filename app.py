...
# Fonction pour récupérer la clé API Mistral
def get_api_key():
    """Récupère la clé API Mistral à partir de diverses sources."""
    ...
    # 3. Essayer le fichier .env s'il existe
    env_path = Path(".env")
    if env_path.exists():
        try:
...
api_key = get_api_key()
if not api_key:
    st.warning("⚠️ Aucune clé API Mistral n'a été trouvée. L'application ne pourra pas analyser les factures.")
...
def process_receipts(prompt_content):
    try:
        api_key = get_api_key()
        if not api_key:
            add_to_log("analysis", "❌ Aucune clé API Mistral valide trouvée")
            return False, 0
        ...
        analyzer = ReceiptAnalyzer(
            ...
            api_key=api_key  # Utiliser la clé API récupérée
        )
...
with tabs[0]:
    ...
    if not st.session_state.receipts_uploaded:
        ...
        with col1:
            ...
            # Afficher l'état de la clé API sans la révéler
            api_key = get_api_key()
            if api_key:
                st.success("✅ Clé API Mistral configurée")
            else:
                st.error("❌ Clé API Mistral non configurée")
                st.markdown("""
                Pour configurer la clé API:
                - Pour le déploiement local: Créez un fichier `.env` contenant `MISTRAL_API_KEY=votre_clé`
                - Pour Streamlit Cloud: Ajoutez un secret nommé `MISTRAL_API_KEY` dans les paramètres
                """)
...

{
    ...
    "payment_method": "Méthode de paiement si disponible"
        ...
        if analyze_button:
            ...
            api_key = get_api_key()
            if not api_key:
                st.error("Aucune clé API Mistral valide trouvée. Veuillez configurer la clé API avant d'analyser les factures.")
            else:
...
# Pied de page
st.markdown("---")
st.markdown("📊 **Analyseur de Factures et Matching Bancaire**t")
